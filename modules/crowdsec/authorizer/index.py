...
def handler(event, context):
    verdict = "Deny"
    cs_client = QueryClient(api_key=os.environ["LAPI_KEY"], lapi_url="http://crowdsec.crowdsec.local:8080/")
    ...
    if action_for_ip == "captcha":
        if event["headers"].get("x-captcha-token") and event["headers"]["x-captcha-token"] not in valid_tokens:
