...
resource "aws_rds_cluster" "csdb" {
  ...
  master_password        = random_password.csdbpassword.result
...
resource "random_password" "csdbpassword" {
  length  = 16
  special = false
  number  = true
}
...
resource "aws_ecs_task_definition" "crowdsec-lapi" {
  ...
  container_definitions = jsonencode([
    {
      name      = "lapi"
      image     = "registry.hub.docker.com/crowdsecurity/crowdsec:v1.2.1"
      cpu       = var.crowdsec_cpu
      memory    = var.crowdsec_memory
      essential = true
      portMappings = [
      ...
      cscli bouncers add lambdaAuthorizer -k ${random_password.bouncer_key.result}&& 
      ./docker_start.sh
      EOF
      ],
      environment = [
...
data "template_file" "cs_config" {
  ...
  vars = {
    db_type     = "postgresql"
    db_user     = "crowdsec"
    db_password = random_password.csdbpassword.result
    db_name     = "crowdsec"
